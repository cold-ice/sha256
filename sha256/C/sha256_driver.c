...
MODULE_AUTHOR("Valerio Lanieri");
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION(DRIVER_NAME ": example software driver for SHA256 XL");
...
/* called on device read. */
static ssize_t sha256_read(struct file *fp, char __user *buf, size_t count, loff_t * f_pos)
{
	uint32_t datak; // read data

	if(count!=4)
	{
		printk(KERN_ERR "ERROR: trying to read an inappropriately sized buffer to sha256. Only accepted size is 4 bytes.\n");
		return -EPERM;
	}

	rmb(); // memory read barrier

  datak = ioread32(base_addr + *f_pos);

	if(copy_to_user(buf,&datak,sizeof(uint32_t))) // copy read data to user space
	{
		return -EACCES; // if error return -EACCES error code
  }
	return count*sizeof(uint8_t); // return number of read bytes
}

/*called on device write*/
static ssize_t sha256_write(struct file *fp, const char __user *buf, size_t count, loff_t *f_pos)
{
  uint32_t bufk;
	...
	if(count!=4)
	{
		printk(KERN_ERR "ERROR: trying to write an inappropriately sized buffer to sha256. Only accepted size is 4 bytes.\n");
		return -EPERM;
	}

	if(copy_from_user(&bufk, buf, sizeof(uint32_t))) // copy data to Kernel space
	{
		return -EACCES;
	}

	iowrite32(bufk, base_addr + *f_pos);
